...
from arguments.config import ModelParams, PipelineParams, get_combined_args
from scene import Scene
...
from hash_encoding.encoding import HashEncoding
from hash_encoding.nn.mlp import MLP
import time
...
def render_set(
    ...
    normal_cam_path = os.path.join(
        model_path, name, "ours_{}".format(iteration), "normal_cam"
    )
    pseudo_normal_path = os.path.join(
        model_path, name, "ours_{}".format(iteration), "pseudo_normal"
    )
    depth_path = os.path.join(model_path, name, "ours_{}".format(iteration), "depth")
    gt_depth_path = os.path.join(
        model_path, name, "ours_{}".format(iteration), "gt_depth"
    )
    depth_var_path = os.path.join(
        model_path, name, "ours_{}".format(iteration), "depth_var"
    )
    opacity_path = os.path.join(
        model_path, name, "ours_{}".format(iteration), "opacity"
    )
    ...
    if gaussians.use_pbr:
        base_color_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "base_color"
        )
        roughness_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "roughness"
        )
        gt_metallic_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "gt_metallic"
        )
        shadow_direction_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "shadow_direction"
        )
        visibility_direct_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "visibility_direct"
        )
        diffuse_light_direct_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "diffuse_light_direct"
        )
        diffuse_direct_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "diffuse_direct"
        )
        specular_direct_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "specular_direct"
        )
        diffuse_env_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "diffuse_env"
        )
        specular_env_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "specular_env"
        )
        pbr_env_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "pbr_env"
        )
        pbr_direct_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "pbr_direct"
        )
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        start = time.time()
        results = render_fn(
            view, gaussians, pipeline, background, dict_params=pbr_kwargs, scene=scene
        )
        ...
        print(f"Render time: {elapsed:.4f}s | Avg: {avg:.4f}s | FPS: {fps:.2f}")

        # Only compute feature visualizations when not in blender mode
        if not is_blender:
        ...
        save_image(
            results["render"], os.path.join(render_path, "{0:05d}.png".format(idx))
        )
        save_image(
            view.original_image.cuda(),
            os.path.join(gts_path, "{0:05d}.png".format(idx)),
        )
        if is_blender == False:
            save_image(
                visualize_depth(results["depth"]),
                os.path.join(depth_path, "{0:05d}.png".format(idx)),
            )
        ...
        save_image(
            (results["depth_var"] / 0.001).clamp_max(1).repeat(3, 1, 1),
            os.path.join(depth_var_path, "{0:05d}.png".format(idx)),
        )
        save_image(
            results["opacity"].repeat(3, 1, 1),
            os.path.join(opacity_path, "{0:05d}.png".format(idx)),
        )
        save_image(
            results["normal"] * 0.5 + 0.5,
            os.path.join(normal_path, "{0:05d}.png".format(idx)),
        )
        # save_image(results["normal_cam"] * 0.5 + 0.5, os.path.join(normal_cam_path, '{0:05d}.png'.format(idx)))
        save_image(
            results["pseudo_normal"] * 0.5 + 0.5,
            os.path.join(pseudo_normal_path, "{0:05d}.png".format(idx)),
        )

        if feature_map_vis is not None:
            save_image(
                feature_map_vis.cuda().permute(2, 0, 1),
                os.path.join(feature_map_path, "{0:05d}.png".format(idx)),
            )
        # if gt_feature_map_vis is not None:
        ...
        if gaussians.use_pbr:
            ...
            save_image(
                results["base_color"],
                os.path.join(base_color_path, "{0:05d}.png".format(idx)),
            )
            save_image(
                results["roughness"].repeat(3, 1, 1),
                os.path.join(roughness_path, "{0:05d}.png".format(idx)),
            )
            ...
            if shadow_direction is not None:
                save_image(
                    ...
                    os.path.join(shadow_direction_path, "{0:05d}.png".format(idx)),
                )
            else:
                save_image(
                    torch.zeros(3, H, W),
            ...
            save_image(
                results["visibility_direct"].repeat(3, 1, 1),
                os.path.join(visibility_direct_path, "{0:05d}.png".format(idx)),
            )
            save_image(
                results["diffuse_light_direct"],
                os.path.join(diffuse_light_direct_path, "{0:05d}.png".format(idx)),
            )
            save_image(
                results["diffuse_direct"],
                os.path.join(diffuse_direct_path, "{0:05d}.png".format(idx)),
            )
            save_image(
                results["specular_direct"],
                os.path.join(specular_direct_path, "{0:05d}.png".format(idx)),
            )
            save_image(
                results["diffuse_env"],
                os.path.join(diffuse_env_path, "{0:05d}.png".format(idx)),
            )
            save_image(
                results["specular_env"],
                os.path.join(specular_env_path, "{0:05d}.png".format(idx)),
            )
            save_image(
                results["pbr_env"],
                os.path.join(pbr_env_path, "{0:05d}.png".format(idx)),
            )
            save_image(
                results["pbr_direct"],
                os.path.join(pbr_direct_path, "{0:05d}.png".format(idx)),
            )
            save_image(
                results["pbr"], os.path.join(pbr_path, "{0:05d}.png".format(idx))
            )
...
def render_sets(
    ...
    with torch.no_grad():
        ...
        if args.checkpoint:
            ...
            iteration = gaussians.create_from_ckpt(
                args.checkpoint, restore_optimizer=True
            )
            ...
            gaussians.create_from_pcd(
                scene.scene_info.point_cloud, scene.cameras_extent
            )
            iteration = scene.loaded_iter
        ...
        if iteration is not None and gaussians.use_pbr:
            ...
            direct_pointlights.create_from_ckpt(
                os.path.join(scene.model_path, "point_light_chkpnt10000.pth"),
            ...
            env_light.create_from_ckpt(
                os.path.join(scene.model_path, "env_light_chkpnt10000.pth"),
                restore_optimizer=False,
            )
            ...
            visibility_env_path = os.path.join(
                dataset.model_path, "visibility_enc_chkpnt10000.pth"
            )
            gaussians.visibility_enc = HashEncoding.load(
                visibility_env_path, mlp=mlp
            ).cuda()

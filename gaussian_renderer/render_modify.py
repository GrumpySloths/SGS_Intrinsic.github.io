...
def render_view(camera: Camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, 
                scaling_modifier=1.0, override_color=None, computer_pseudo_normal=True):
    ...
    shs = None
    colors_precomp = None
    if override_color is None:  #在本次的train的代码实际调用中override_color是None
        if pipe.convert_SHs_python:
            ...
            shs = pc.get_features  #实际代码中直接获取features,让 rasterizer去做SH到RGB的转换
    else:
        colors_precomp = override_color
        
    normals = pc.get_normal
    # 额外添加属性关于 albedo, roughness, metallic
    ...
    # print("normals before cat shape:",normals.shape) 
    # print("depths before cat shape:",depths.shape)
    # print("depths2 before cat shape:",depths2.shape)
    # print("albedo before cat shape:",albedo.shape)
    # print("roughness before cat shape:",roughness.shape)
    # print("metallic before cat shape:",metallic.shape)

    # 将 albedo, roughness, metallic 添加到 features 中
    features = torch.cat([normals, depths, depths2, albedo, roughness, metallic], dim=-1)

    # print("features before cat shape:",features.shape)
    ...
    # print("rendered_opacity.shape:",rendered_opacity.shape)
    # print("rendered_depth.shape:",rendered_depth.shape)
    # print("rendered_pseudo_normal.shape:",rendered_pseudo_normal.shape)
    # print("rendered_surface_xyz.shape:",rendered_surface_xyz.shape)
    # print("rendered_feature.shape:",rendered_feature.shape)
    # print("num_rendered:",num_rendered)
    # print("num_contrib:",num_contrib)
    mask = num_contrib > 0
    ...
    # 根据新的 features 结构拆分 rendered_feature
    rendered_normal, rendered_depth, rendered_depth2, rendered_albedo, rendered_roughness, rendered_metallic = torch.split(
        rendered_feature, [3, 1, 1, 3,1,1], dim=0
    )
...
#该版本的render_view_rgbx函数与render_view函数的区别在于它额外将tone_mapping后的gaussian额外进行了一次光栅化的过程
def render_view_rgbx(camera: Camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, 
                scaling_modifier=1.0, override_color=None, computer_pseudo_normal=True):
    # Create zero tensor. We will use it to make pytorch return gradients of the 2D (screen-space) means

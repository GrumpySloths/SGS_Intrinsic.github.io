...
import sys
sys.path.append("/home/jiahao/ipsm_relighting_v4")
from hash_encoding.nn import Module, ParameterModule
...
class HashEncoding(nn.Module):
    ...
    def save(self, path: str) -> None:
        """Save the encoding module to a file."""
        state = {
        ...
        if self.backend == "torch" and self.hash_table is not None:
            state['hash_table'] = self.hash_table.detach().cpu()
        elif self.backend == "tcnn" and hasattr(self, "encoder"):
            # 保存 tcnn 编码器参数
            state['encoder'] = self.encoder.state_dict()
        torch.save(state, path)

    @classmethod
    def load(cls, path: str, mlp: nn.Module = None) -> "HashEncoding":
        """Load the encoding module from a file."""
        state = torch.load(path, map_location='cpu')
        obj = cls(
            mlp=mlp if mlp is not None else nn.Identity(),
            num_levels=state['num_levels'],
            min_res=state['min_res'],
            max_res=state['max_res'],
            log2_hashmap_size=state['log2_hashmap_size'],
            features_per_level=state['features_per_level'],
            hash_init_scale=state['hash_init_scale'],
            backend=state['backend'],
            interpolation=state['interpolation'],
            grad_scaling=state['grad_scaling'],
            scale=state['scale'],
        )
        # obj.__setup__()
        if mlp is not None:
            obj.mlp.load_state_dict(state['mlp'])
        if obj.backend == "torch" and 'hash_table' in state:
            obj.hash_table.copy_(state['hash_table'])
        elif obj.backend == "tcnn" and 'encoder' in state and hasattr(obj, "encoder"):
            # 加载 tcnn 编码器参数
            obj.encoder.load_state_dict(state['encoder'])
        return obj
    ...
    def __call__(self, means: Float[Tensor, "N 3"], light_positions: Float[Tensor, "spp 3"]) -> Float[Tensor, "*bs output_dim"]:
        # means: (N, 3), light_positions: (spp, 3)
        N = means.shape[0]
        spp = light_positions.shape[0]

        # Preprocess means and light_positions
        means_norm = (means / self.scale).clamp(-1, 1)              # (N, 3)
        light_positions_norm = (light_positions / self.scale).clamp(-1, 1)  # (spp, 3)

        # Compute direction and relative distance
        means_exp = means_norm.unsqueeze(0).expand(spp, N, 3)         # (spp, N, 3)
        light_exp = light_positions_norm.unsqueeze(1).expand(spp, N, 3)    # (spp, N, 3)

        direction = F.normalize(means_exp - light_exp, dim=-1)        # (spp, N, 3)
        rel_dist = torch.norm(means_exp - light_exp, dim=-1, keepdim=True)  # (spp, N, 1)

        # Encode means
        if self.grad_scaling is not None:
            means_enc = means_norm * (1 / self.grad_scaling) + means_norm.detach() * (1 - 1 / self.grad_scaling)
        else:
            means_enc = means_norm

        if self.backend == 'tcnn':
            feats = self.encoder(means_enc)  # (N, features)
        else:
            feats = self.pytorch_fwd(means_enc)  # (N, features)

        if self.grad_scaling is not None:
            feats = feats * self.grad_scaling + feats.detach() * (1 - self.grad_scaling)

        # Expand feats to match (spp, N, features)
        feats_exp = feats.unsqueeze(0).expand(spp, N, feats.shape[-1])  # (spp, N, features)

        # Concatenate features, direction, and rel_dist
        mlp_input = torch.cat([feats_exp, direction, rel_dist], dim=-1)  # (spp, N, features+3+1)

        # Flatten for MLP input if needed
        mlp_input_flat = mlp_input.view(-1, mlp_input.shape[-1])  # (spp*N, input_dim)
        output = self.mlp(mlp_input_flat)  # (spp*N, output_dim)
        output = output.view(spp, N, -1)   # (spp, N, output_dim)
        return output
    
    def forward_defer(self, means: Float[Tensor, "3 h w"], light_positions: Float[Tensor, "spp 3"]) -> Float[Tensor, "spp output_dim h w"]:
        # means: (3, h, w), light_positions: (spp, 3)
        C, h, w = means.shape
        spp = light_positions.shape[0]

        # Reshape means to (h*w, 3)
        means_reshaped = means.permute(1, 2, 0).reshape(-1, 3)  # (h*w, 3)
        means_norm = (means_reshaped / self.scale).clamp(-1, 1)  # (h*w, 3)
        light_positions_norm = (light_positions / self.scale).clamp(-1, 1)  # (spp, 3)

        # Compute direction and relative distance
        means_exp = means_norm.unsqueeze(0).expand(spp, h * w, 3)         # (spp, h*w, 3)
        light_exp = light_positions_norm.unsqueeze(1).expand(spp, h * w, 3)    # (spp, h*w, 3)

        direction = F.normalize(means_exp - light_exp, dim=-1)        # (spp, h*w, 3)
        rel_dist = torch.norm(means_exp - light_exp, dim=-1, keepdim=True)  # (spp, h*w, 1)
        ...
        if self.backend == 'tcnn':
            feats = self.encoder(means_enc)  # (h*w, features)
        else:
            feats = self.pytorch_fwd(means_enc)  # (h*w, features)

        if self.grad_scaling is not None:
            feats = feats * self.grad_scaling + feats.detach() * (1 - self.grad_scaling)

        # Expand feats to match (spp, h*w, features)
        feats_exp = feats.unsqueeze(0).expand(spp, h * w, feats.shape[-1])  # (spp, h*w, features)

        # Concatenate features, direction, and rel_dist
        mlp_input = torch.cat([feats_exp, direction, rel_dist], dim=-1)  # (spp, h*w, features+3+1)

        # Flatten for MLP input if needed
        mlp_input_flat = mlp_input.view(-1, mlp_input.shape[-1])  # (spp*h*w, input_dim)
        output = self.mlp(mlp_input_flat)  # (spp*h*w, output_dim)
        output = output.view(spp, h, w, -1).permute(0, 3, 1, 2).contiguous()   # (spp, output_dim, h, w)
        return output

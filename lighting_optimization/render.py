...
class IIR_SSRT_RenderLayer(nn.Module):
    def __init__(self,
        ...
        self.use_ssrt = use_ssrt #false
        self.use_specular = use_specular #true
    ...
    def forward(
        ...
        """
        bn, _, row, col = albedo.shape #[1,3,240,320]
        ...
        direction = direction.permute(0, 1, 3, 4, 2)  # (bn, spp, h, w, 3),spp为48+1
        direction = direction.permute(1, 0, 2, 3, 4)  # (spp, bn, h, w, 3)
        direction = direction.reshape(lighting_model.spp, -1, 3) #[48+1,76800,3]

...
from utils.pose_utils import (
    generate_random_poses_llff_annealing_view,
...
class Scene:
    ...
    def __init__(
        ...
        )  # Initialize the pseudo-view interpolator.
        self.pseudo_sampling_num = (
            20  # Number of pseudo views generated for each train camera.
        )
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            if (
                args.source_path.find("vggt") != -1
                or args.source_path.find("interiorverse") != -1
            ):
                scene_info = sceneLoadTypeCallbacks["Colmap_vggt"](
                    args.source_path,
                    args.images,
                    args.eval,
                    "vggt",
                    args.n_views,
                    use_pbr=gaussians.use_pbr,
                )
        elif os.path.exists(os.path.join(args.source_path, "transforms_train.json")):
            print("Found transforms_train.json file, assuming Blender data set!")
            scene_info = sceneLoadTypeCallbacks["Blender"](
                args.source_path, args.white_background, args.eval, args.n_views
            )
        else:
        ...
        # Pseudo camera settings: use the interpolator to generate two sequences for each train camera.
        # One sequence interpolates between the nearest train camera and the nearest test camera for later SAM region alignment.
        self.camtoworlds_test = self.get_c2ws_test()
        self.camtoworlds_train = self.get_c2ws_train()
        interpolate_pseudo_test_poses = self.interpolator.interpolate_poses(
            self.camtoworlds_test, self.camtoworlds_train, self.pseudo_sampling_num
        )  # [N,40,4,4]
        interpolate_pseudo_train_poses, neighbor_indices = (
            self.interpolator.interpolate_training_pose_neighbors(
                self.camtoworlds_train, self.pseudo_sampling_num
            )
        )  # [N,40,4,4]
        ...
        print(
            "interpolate_pseudo_test_poses shape:", interpolate_pseudo_test_poses.shape
        )
        print(
            "interpolate_pseudo_train_poses shape:",
            interpolate_pseudo_train_poses.shape,
        )
        ...
        for i in range(interpolate_pseudo_test_poses.shape[0]):
            ...
            for j in range(interpolate_pseudo_test_poses.shape[1]):
                ...
                pose = np.linalg.inv(pose)  # Convert to w2c.
                cam = PseudoCamera(
                    R=pose[:3, :3].T,
                    T=pose[:3, 3],
                    FoVx=FoVx,
    ...
    def sample_random_pseudo_cameras(self, id):
        """
        Randomly sample one PseudoCamera from interpolate_pseudo_test_cams[id] and interpolate_pseudo_train_cams[id].
        :param id: Index
        :return: (pseudo_test_cam, pseudo_train_cam)
        """
        if id >= len(self.interpolate_pseudo_test_cams) or id >= len(
            self.interpolate_pseudo_train_cams
        ):
            raise IndexError("id exceeds the pseudo camera sequence length")
    ...
    def get_c2ws_train(self, scale=1.0):
        # Return a NumPy array with shape [N, 4, 4].
        c2ws = []
        for viewpoint_cam in self.train_cameras[scale]:
            c2w = (
                torch.inverse(viewpoint_cam.world_view_transform.T).cpu().numpy()
            )  # [4, 4]
            c2ws.append(c2w)

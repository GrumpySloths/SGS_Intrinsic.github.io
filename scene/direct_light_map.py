...
class DirectLightMap:
    ...
    def direct_light_with_augmentation(self, dirs, mask_prob=0.5, intensity_range=(0.5, 1.5)):
        shape = dirs.shape
        dirs = dirs.reshape(-1, 3)
        ...
        # 2. 随机强度变化
        # 生成随机强度因子
        intensity_factor = torch.rand(1, device=envir_map.device) * \
                        (intensity_range[1] - intensity_range[0]) + intensity_range[0]
        
        # 应用强度变化 - 标量乘法，梯度可以正常回传
    ...
    def direct_light_with_patch_augmentation(self, dirs, mask_patch_range=(16, 32), mask_patch_num=2, intensity_patch_num=2, intensity_range=(0.5, 1.5)):
        shape = dirs.shape
        dirs = dirs.reshape(-1, 3)
        ...
        # 2. 随机选取intensity_patch_num个patch块改变强度
        for _ in range(intensity_patch_num):
            patch_h = np.random.randint(mask_patch_range[0], mask_patch_range[1]+1)
            patch_w = np.random.randint(mask_patch_range[0], mask_patch_range[1]+1)
            y0 = np.random.randint(0, H - patch_h + 1)
            x0 = np.random.randint(0, W - patch_w + 1)
            intensity_factor = torch.rand(1, device=envir_map.device) * (intensity_range[1] - intensity_range[0]) + intensity_range[0]
    ...
    def direct_light_with_augmentation_batch(self, dirs, batch, mask_prob=0.5, intensity_range=(0.5, 1.5)):
        shape = dirs.shape
        dirs = dirs.reshape(-1, 3)
        ...
        # 生成随机掩码 - 每个batch独立
        mask = (torch.rand(batch, 1, envir_map.shape[2], envir_map.shape[3], device=envir_map.device) > mask_prob).float()  # [B, 1, H, W]
        envir_map = envir_map * mask  # [B, 3, H, W]

        # 随机强度变化 - 每个batch独立
        intensity_factor = torch.rand(batch, 1, 1, 1, device=envir_map.device) * \
            (intensity_range[1] - intensity_range[0]) + intensity_range[0]
        ...
        query_y = (phi / np.pi) * 2 - 1
        query_x = - theta / np.pi
        grid = torch.stack((query_x, query_y)).permute(1, 0).unsqueeze(0).unsqueeze(0)  # [1, 1, N, 2]
        grid = grid.expand(batch, -1, -1, -1)  # [B, 1, N, 2]

        # grid_sample: [B, 3, H, W], grid: [B, 1, N, 2] -> [B, 3, 1, N]
        light_rgbs = F.grid_sample(envir_map, grid, align_corners=True).squeeze(2).permute(0, 2, 1)  # [B, N, 3]

...
class GaussianModel:
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, albedo, roughness, metallic), axis=1)
    ...
    def densification_postfix(self, new_xyz, new_normal, new_features_dc, new_features_rest, new_opacities, new_scaling,
                              new_rotation, new_albedo, new_roughness, new_metallic):
        d = {"xyz": new_xyz,
    ...
    def proximity(self, scene_extent, N = 3):
        ...
        new_albedo = self._albedo[new_indices]
        new_roughness = self._roughness[new_indices]
        new_metallic = self._metallic[new_indices]
        self.densification_postfix(new_xyz, new_normal, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_albedo, new_roughness, new_metallic)
    ...
    def densify_and_split_origin(self, grads, grad_threshold, scene_extent, iter, N=2):
        ...
        new_features_rest = self._features_rest[selected_pts_mask].repeat(N,1,1)
        new_opacity = self._opacity[selected_pts_mask].repeat(N,1)
        new_normal = self._normal[selected_pts_mask].repeat(N, 1, 1)
        new_albedo = self._albedo[selected_pts_mask].repeat(N, 1)
        new_roughness = self._roughness[selected_pts_mask].repeat(N, 1)
        new_metallic = self._metallic[selected_pts_mask].repeat(N, 1)

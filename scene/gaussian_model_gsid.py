...
from simple_knn_r3dg._C import distCUDA2
...
class GaussianModel:
    ...
    def setup_functions(self):
        ...
        if self.use_pbr:
            ...
            self.base_color_activation = lambda x: torch.sigmoid(x) * 0.9 + 0.09
            ...
            self.shadowdirection_activation = lambda x: torch.nn.functional.normalize(x, dim=-1, eps=1e-3)

    def __init__(self, sh_degree: int, render_type='render'):
        self.render_type = render_type
        self.use_pbr = render_type in ['neilf','gsid']
        ...
        if self.use_pbr:
            ...
            self._shadowdirection = torch.empty(0)
    ...
    def restore(self, model_args, training_args,
        ...
        (self.active_sh_degree,
         ...
         denom,
         opt_dict,
         self.spatial_lr_scale) = model_args[:16]
    ...
    @property
    def get_shadowdirection(self):
        return self.shadowdirection_activation(self._shadowdirection)

    @property
    def get_brdf(self):
        if self.use_pbr:
            return torch.cat([self.get_base_color, self.get_roughness, self.get_shadowdirection], dim=-1)
        else:
    ...
    def attribute_names(self):
        attribute_names = ['xyz', 'normal', 'shs_dc', 'shs_rest', 'scaling', 'rotation', 'opacity', 'semantic_feature']
        if self.use_pbr:
    ...
    def create_from_ckpt(self, checkpoint_path, restore_optimizer=False):
        ...
        # Initialize semantic features if not present in checkpoint
        if self._semantic_feature is None or self._semantic_feature.numel() == 0:
            semantic_feature_size = 512
            self._semantic_feature = nn.Parameter(torch.zeros(
                self._xyz.shape[0], semantic_feature_size, 1, dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True))
        ...
        if self.use_pbr:
            if len(model_args) > 16:
                ...
                self._shadowdirection = nn.Parameter(torch.zeros_like(self._xyz).requires_grad_(True))
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        semantic_feature_size = 512  # Default semantic feature size
        # semantic_feature_size = int(semantic_feature_size/8) #64
        semantic_feature_size = int(semantic_feature_size/16) #32
        print("semantic_feature_size", semantic_feature_size)
        ...
        if self.use_pbr:
            ...
            shadowdirection = torch.zeros_like(fused_point_cloud) #这里的实现可能还有点大问题，要修改一下,这里看看将其设置为光源位置感知的mlp

            self._base_color = nn.Parameter(base_color.requires_grad_(True))
            self._roughness = nn.Parameter(roughness.requires_grad_(True))
            self._shadowdirection = nn.Parameter(shadowdirection.requires_grad_(True))
    ...
    def training_setup(self, training_args: OptimizationParams):
        ...
        if self.use_pbr:
            ...
            l.extend([
                ...
                {'params': [self._shadowdirection], 'lr': training_args.base_color_lr, "name": "shadowdirection"}
            ])
        ...
        # 假设你已经初始化了 optimizer,这里做一个参考，用于后续将学习率置0
        # for param_group in self.optimizer.param_groups:
        #     if param_group["name"] in ["xyz", "normal"]:
        #         param_group["lr"] = 0.0
    def step(self):
        self.optimizer.step()
    ...
    def construct_list_of_attributes(self):
        ...
        if self.use_pbr:
            ...
            l.append('roughness')
            for i in range(self._shadowdirection.shape[1]):
                l.append('shadowdirection_{}'.format(i))
        return l
    ...
    def save_ply(self, path):
        ...
        attributes_list = [xyz, normal, sh_dc, sh_rest, opacities, scale, rotation, semantic_feature]
        if self.use_pbr:
            attributes_list.extend([
                self._base_color.detach().cpu().numpy(),
                self._roughness.detach().cpu().numpy(),
                self._shadowdirection.detach().cpu().numpy(),
            ])
    ...
    def load_ply(self, path):
        ...
        if semantic_feature_names:
            ...
            # Determine the shape based on the number of features
            feature_dim = int(np.sqrt(len(semantic_feature_names))) if len(semantic_feature_names) > 0 else 1
            semantic_features = semantic_features.reshape((semantic_features.shape[0], feature_dim, -1))
            self._semantic_feature = nn.Parameter(torch.tensor(
                semantic_features, dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True))
        else:
            # Initialize with default semantic features if not found in file
            semantic_feature_size = 512
            self._semantic_feature = nn.Parameter(torch.zeros(
                xyz.shape[0], semantic_feature_size, 1, dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True))
        ...
        if self.use_pbr:
            ...
            shadowdirection_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("shadowdirection")]
            shadowdirection_names = sorted(shadowdirection_names, key=lambda x: int(x.split('_')[-1]))
            shadowdirection = np.zeros((xyz.shape[0], len(shadowdirection_names)))
            for idx, attr_name in enumerate(shadowdirection_names):
                shadowdirection[:, idx] = np.asarray(plydata.elements[0][attr_name])
            ...
            self._shadowdirection = nn.Parameter(
                torch.tensor(shadowdirection, dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def prune_points(self, mask):
        ...
        if self.use_pbr:
            ...
            self._shadowdirection = optimizable_tensors["shadowdirection"]
    ...
    def densification_postfix(self, new_xyz, new_normal, new_shs_dc, new_shs_rest, new_opacities, new_scaling,
                              new_rotation, new_semantic_feature=None, new_base_color=None, new_roughness=None,
                              new_incidents_dc=None, new_incidents_rest=None, new_shadowdirection=None):
        d = {"xyz": new_xyz,
             "normal": new_normal,
        ...
        if self.use_pbr:
            d.update({
                ...
                "shadowdirection": new_shadowdirection,
            })
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, grads_normal, grad_normal_threshold, N=2):
        ...
        args = [new_xyz, new_normal, new_shs_dc, new_shs_rest, new_opacity, new_scaling, new_rotation, new_semantic_feature]
        if self.use_pbr:
            new_base_color = self._base_color[selected_pts_mask].repeat(N, 1)
            new_roughness = self._roughness[selected_pts_mask].repeat(N, 1)
            new_shadowdirection = self._shadowdirection[selected_pts_mask].repeat(N, 1)
            args.extend([
                new_base_color,
                new_roughness,
                None,  # new_incidents_dc
                None,  # new_incidents_rest
                new_shadowdirection,
            ])
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, grads_normal, grad_normal_threshold):
        ...
        args = [new_xyz, new_normal, new_shs_dc, new_shs_rest, new_opacities,
                new_scaling, new_rotation, new_semantic_feature]
        if self.use_pbr:
            new_base_color = self._base_color[selected_pts_mask]
            new_roughness = self._roughness[selected_pts_mask]
            new_shadowdirection = self._shadowdirection[selected_pts_mask]
            args.extend([
                new_base_color,
                new_roughness,
                None,  # new_incidents_dc

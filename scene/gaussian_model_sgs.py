...
class GaussianModel(nn.Module):
    def setup_functions(self):
        ...
        self.normal_activation = lambda x: torch.nn.functional.normalize(
            x, dim=-1, eps=1e-3
        )
        ...
        if self.use_pbr:
            ...
            self.inverse_roughness_activation = lambda y: inverse_sigmoid(
                (y - 0.09) / 0.9
            )
    ...
    def __init__(self, sh_degree: int, render_type="render"):
        ...
        self.use_pbr = render_type in ["neilf", "sgs"]
        ...
        if self.use_pbr:
            ...
            self.visibility_enc = HashEncoding(
                mlp=MLP(
                    layers=[-1, 32, 32, 1],
                    activation="sigmoid",
                    bias=False,
    ...
    def restore(
        ...
        if len(model_args) > 16 and self.use_pbr:
            tail = model_args[16:]
            if len(tail) >= 2:
                self._base_color, self._roughness = tail[:2]
    ...
    def get_base_color(self):
        return (
            self.base_color_activation(self._base_color)
            ...
            if len(model_args) > 16:
                ...
                if len(tail) >= 2:
                    ...
                    self._base_color = nn.Parameter(
                        torch.zeros_like(self._xyz).requires_grad_(True)
                    )
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        self._shs_dc = nn.Parameter(
            shs[:, :, 0:1].transpose(1, 2).contiguous().requires_grad_(True)
        )
        self._shs_rest = nn.Parameter(
    ...
    def training_setup(self, training_args: OptimizationParams):
        ...
        self.normal_gradient_accum = torch.zeros(
        ...
        l = [
            ...
            {
                "params": [self._shs_rest],
                "lr": training_args.sh_lr / 20.0,
                "name": "f_rest",
            },
            {
        ...
        if self.use_pbr:
            ...
            l.extend(
                [
                    {
                        "params": [self._base_color],
                        "lr": training_args.base_color_lr,
                        "name": "base_color",
                    },
                    {
        ...
        if self.use_pbr:
            self.visibility_enc_optimizer = torch.optim.Adam(
                self.visibility_enc.parameters(),
                lr=training_args.visibility_enc_lr,
                eps=1e-15,
            )
        ...
        # Assuming the optimizer has already been initialized; this is a reference for setting the learning rate to zero later.
    ...
    def step(self):
        ...
        if self.use_pbr:
            self.visibility_enc_optimizer.step()
            self.visibility_enc_optimizer.zero_grad()
    ...
    def load_ply(self, path):
        ...
        self._shs_dc = nn.Parameter(
            torch.tensor(shs_dc, dtype=torch.float, device="cuda")
            .transpose(1, 2)
            .contiguous()
            .requires_grad_(True)
        )
        ...
        semantic_feature_names = [
            p.name
            for p in plydata.elements[0].properties
            if p.name.startswith("semantic_feature_")
        ]
        if semantic_feature_names:
            semantic_feature_names = sorted(
                semantic_feature_names, key=lambda x: int(x.split("_")[-1])
            )
            ...
            feature_dim = (
                int(np.sqrt(len(semantic_feature_names)))
                if len(semantic_feature_names) > 0
                else 1
            )
            semantic_features = semantic_features.reshape(
                (semantic_features.shape[0], feature_dim, -1)
            )
            self._semantic_feature = nn.Parameter(
                torch.tensor(semantic_features, dtype=torch.float, device="cuda")
                .transpose(1, 2)
                .contiguous()
                .requires_grad_(True)
            )
        ...
        if self.use_pbr:
            ...
            base_color_names = sorted(
                base_color_names, key=lambda x: int(x.split("_")[-1])
            )
            ...
            self._base_color = nn.Parameter(
                torch.tensor(
                    base_color, dtype=torch.float, device="cuda"
                ).requires_grad_(True)
            )
    ...
    def densification_postfix(
        ...
        new_base_color=None,
        new_roughness=None,
        new_incidents_dc=None,
        new_incidents_rest=None,
    ):
        d = {
            "xyz": new_xyz,
            "normal": new_normal,
        ...
        self.weights_accum = torch.cat(
            [self.weights_accum, torch.ones((new_xyz.shape[0], 1), device="cuda")],
            dim=0,
        )
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.normal_gradient_accum = torch.zeros(
    ...
    def densify_and_split(
        ...
        selected_pts_mask_normal = torch.where(
            padded_grad_normal >= grad_normal_threshold, True, False
        )
        print(
            "densify_and_split_normal:",
            selected_pts_mask_normal.sum().item(),
            "/",
            self.get_xyz.shape[0],
        )

        selected_pts_mask = torch.logical_or(
            selected_pts_mask, selected_pts_mask_normal
        )
    ...
    def densify_and_clone(
        self, grads, grad_threshold, scene_extent, grads_normal, grad_normal_threshold
    ):
        # Extract points that satisfy the gradient condition
        ...
        selected_pts_mask_normal = torch.where(
            torch.norm(grads_normal, dim=-1) >= grad_normal_threshold, True, False
        )

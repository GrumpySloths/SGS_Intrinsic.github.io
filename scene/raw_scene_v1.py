...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        interpolate_pseudo_firstNeighbor_poses, _, interpolate_pseudo_secondNeighbor_poses, _ = \
                self.interpolator.interpolate_training_pose_neighbors_with_second(self.camtoworlds_train, self.pseudo_sampling_num)
        interpolate_pseudo_firstNeighbor_poses=np.array(interpolate_pseudo_firstNeighbor_poses) # [N_train, 40, 4, 4]
        interpolate_pseudo_secondNeighbor_poses=np.array(interpolate_pseudo_secondNeighbor_poses) # [N_train, 40, 4, 4]
        
        self.interpolate_pseudo_first_cams = []
        self.interpolate_pseudo_second_cams = []
        ...
        for i in range(interpolate_pseudo_firstNeighbor_poses.shape[0]):
            sequence_cams = []
            for j in range(interpolate_pseudo_firstNeighbor_poses.shape[1]):
                pose = interpolate_pseudo_firstNeighbor_poses[i, j]
                #对pose进行一个求逆操作
                ...
                cam = PseudoCamera(
                    ...
                    width=width,
                    height=height,
                    id=(i * interpolate_pseudo_firstNeighbor_poses.shape[1] + j),
                )
                sequence_cams.append(cam)
            self.interpolate_pseudo_first_cams.append(sequence_cams)

        # Convert train pseudo poses to PseudoCamera
        for i in range(interpolate_pseudo_secondNeighbor_poses.shape[0]):
            sequence_cams = []
            for j in range(interpolate_pseudo_secondNeighbor_poses.shape[1]):
                pose = interpolate_pseudo_secondNeighbor_poses[i, j]
                pose = np.linalg.inv(pose)
                cam = PseudoCamera(
                    R=pose[:3, :3].T,
                    T=pose[:3, 3],
                    ...
                    width=width,
                    height=height,
                    id=(i * interpolate_pseudo_secondNeighbor_poses.shape[1] + j),
                )
                sequence_cams.append(cam)
            self.interpolate_pseudo_second_cams.append(sequence_cams)
    ...
    def sample_random_pseudo_cameras_v2(self,id):
        """
        随机从interpolate_pseudo_test_cams[id]和interpolate_pseudo_train_cams[id]中各采样一个PseudoCamera
        :param id: 索引
        :return: (pseudo_test_cam, pseudo_train_cam)
        """
        if id >= len(self.interpolate_pseudo_first_cams) or id >= len(self.interpolate_pseudo_second_cams):
            raise IndexError("id 超出 pseudo camera 序列长度")
        pseudo_first_seq = self.interpolate_pseudo_first_cams[id]
        pseudo_second_seq = self.interpolate_pseudo_second_cams[id]
        pseudo_first_cam = random.choice(pseudo_first_seq)
        pseudo_second_cam = random.choice(pseudo_second_seq)
        return pseudo_first_cam, pseudo_second_cam 

...
do
    ...
    python "${ROOT_DIR}/eval_nvs.py" --eval \
        -m ${OUTPUT_DIR}/${dataset_name}/${name}_neilf_nv${n_views}_sgs_${version}_morelight \
        -c ${OUTPUT_DIR}/${dataset_name}/${name}_neilf_nv${n_views}_sgs_${version}_morelight/chkpnt10000.pth \
        -t sgs \
        --n_views ${n_views}
done

...
do
    ...
    python "${ROOT_DIR}/train.py" --eval \
        -s ${DATA_ROOT_DIR}/${dataset_name}/${name}/ \
        -m ${OUTPUT_DIR}/${dataset_name}/${name}_neilf_nv${n_views}_sgs_${version}_wodeshadow \
        -c ${OUTPUT_DIR}/${dataset_name}/${name}_3dgs_nv${n_views}_${version}/chkpnt7000.pth \
        --save_training_vis \
        --position_lr_init 0 \
        --position_lr_final 0 \
    ...
    python "${ROOT_DIR}/eval_nvs.py" --eval \
        -m ${OUTPUT_DIR}/${dataset_name}/${name}_neilf_nv${n_views}_sgs_${version}_wodeshadow \
        -c ${OUTPUT_DIR}/${dataset_name}/${name}_neilf_nv${n_views}_sgs_${version}_wodeshadow/chkpnt10000.pth \
        -t sgs \
        --n_views ${n_views}
done

...
do
    ...
    python "${ROOT_DIR}/train.py" --eval \
        ...
        -m ${OUTPUT_DIR}/${dataset_name}/${name}_neilf_nv${n_views}_sgs_${version}_vis_enc_self \
        -c ${OUTPUT_DIR}/${dataset_name}/${name}_3dgs_nv${n_views}_v5/chkpnt7000.pth \
        --save_training_vis \
        --position_lr_init 0 \
        --position_lr_final 0 \
        --normal_lr 0 \
    ...
    python "${ROOT_DIR}/eval_nvs.py" --eval \
        -m ${OUTPUT_DIR}/${dataset_name}/${name}_neilf_nv${n_views}_sgs_${version}_vis_enc_self \
        -c ${OUTPUT_DIR}/${dataset_name}/${name}_neilf_nv${n_views}_sgs_${version}_vis_enc_self/chkpnt10000.pth \
        -t sgs \
        --n_views ${n_views}
done
